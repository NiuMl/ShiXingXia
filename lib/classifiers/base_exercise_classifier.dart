...
/// Abstract base class for exercise classifiers
/// Contains shared logic for pose classification and repetition counting
abstract class BaseExerciseClassifier {
  ...
  final ValueNotifier<int> repetitionCounter = ValueNotifier<int>(0);
  ...
  final ValueNotifier<double> enterThreshold;
  final ValueNotifier<double> exitThreshold;
  ...
  BaseExerciseClassifier({
    required this.config,
    this.logEveryXFrames = 1,
  })  : enterThreshold = ValueNotifier<double>(config.defaultEnterThreshold),
        exitThreshold = ValueNotifier<double>(config.defaultExitThreshold);
  ...
  bool _isBodyHorizontal(Map<PoseLandmarkType, PoseLandmark> lms) {
    // Only check if this exercise requires horizontal position
    if (!config.requiresHorizontalPosition) {
    ...
    final angleFromHorizontal =
        asin((verticalDist / bodyLength).clamp(0.0, 1.0)) * 180 / pi;

    currentBodyAngle.value = angleFromHorizontal;

    // Check if body is within horizontal threshold
    final horizontal = angleFromHorizontal >= config.horizontalAngleMin! &&
        angleFromHorizontal <= config.horizontalAngleMax!;
  ...
  Map<String, double> extractFeatures(Pose pose) {
    ...
    final distances = {
      'left_shoulder_left_wrist': _distance3D(
          get(PoseLandmarkType.leftShoulder), get(PoseLandmarkType.leftWrist)),
      'right_shoulder_right_wrist': _distance3D(
          get(PoseLandmarkType.rightShoulder),
          get(PoseLandmarkType.rightWrist)),
      'left_hip_left_ankle': _distance3D(
          get(PoseLandmarkType.leftHip), get(PoseLandmarkType.leftAnkle)),
      'right_hip_right_ankle': _distance3D(
          get(PoseLandmarkType.rightHip), get(PoseLandmarkType.rightAnkle)),
      'left_hip_left_wrist': _distance3D(
          get(PoseLandmarkType.leftHip), get(PoseLandmarkType.leftWrist)),
      'right_hip_right_wrist': _distance3D(
          get(PoseLandmarkType.rightHip), get(PoseLandmarkType.rightWrist)),
      'left_shoulder_left_ankle': _distance3D(
          get(PoseLandmarkType.leftShoulder), get(PoseLandmarkType.leftAnkle)),
      'right_shoulder_right_ankle': _distance3D(get(PoseLandmarkType.rightShoulder),
          get(PoseLandmarkType.rightAnkle)),
      'left_hip_right_wrist': _distance3D(
          get(PoseLandmarkType.leftHip), get(PoseLandmarkType.rightWrist)),
      'right_hip_left_wrist': _distance3D(
          get(PoseLandmarkType.rightHip), get(PoseLandmarkType.leftWrist)),
      'left_elbow_right_elbow': _distance3D(
          get(PoseLandmarkType.leftElbow), get(PoseLandmarkType.rightElbow)),
      'left_knee_right_knee': _distance3D(
          get(PoseLandmarkType.leftKnee), get(PoseLandmarkType.rightKnee)),
      'left_wrist_right_wrist': _distance3D(
          get(PoseLandmarkType.leftWrist), get(PoseLandmarkType.rightWrist)),
      'left_ankle_right_ankle': _distance3D(
          get(PoseLandmarkType.leftAnkle), get(PoseLandmarkType.rightAnkle)),
      'left_hip_avg_left_wrist_left_ankle': _distance3D(
        get(PoseLandmarkType.leftHip),
        _avgLandmarks([
          get(PoseLandmarkType.leftWrist),
          get(PoseLandmarkType.leftAnkle)
        ]),
      ),
    ...
    final angles = {
      'right_elbow_right_shoulder_right_hip': _angle3D(
          get(PoseLandmarkType.rightElbow),
          get(PoseLandmarkType.rightShoulder),
          get(PoseLandmarkType.rightHip)),
      'left_elbow_left_shoulder_left_hip': _angle3D(
          get(PoseLandmarkType.leftElbow),
          get(PoseLandmarkType.leftShoulder),
          get(PoseLandmarkType.leftHip)),
      ...
      'right_hip_right_knee_right_ankle': _angle3D(
          get(PoseLandmarkType.rightHip),
          get(PoseLandmarkType.rightKnee),
          get(PoseLandmarkType.rightAnkle)),
      'left_hip_left_knee_left_ankle': _angle3D(get(PoseLandmarkType.leftHip),
          get(PoseLandmarkType.leftKnee), get(PoseLandmarkType.leftAnkle)),
      'right_wrist_right_elbow_right_shoulder': _angle3D(
          get(PoseLandmarkType.rightWrist),
          get(PoseLandmarkType.rightElbow),
          get(PoseLandmarkType.rightShoulder)),
      'left_wrist_left_elbow_left_shoulder': _angle3D(
          get(PoseLandmarkType.leftWrist),
          get(PoseLandmarkType.leftElbow),
          get(PoseLandmarkType.leftShoulder)),
    };
  ...
  Future<Map<String, dynamic>> classifyPose(Pose pose) async {
    ...
    try {
      // Check if body position requirements are met
      final isValidPosition = _isBodyHorizontal(pose.landmarks);
      ...
      if (!isValidPosition && config.requiresHorizontalPosition) {
        ...
        currentPose.value = 'invalid_position';

        return {
          'pose': 'invalid_position',
          'confidence': 0.0,
          'down_confidence': 0.0,
          'up_confidence': 0.0,
          'error': 'Body not in valid position',
        };
      }
      ...
      _smoothedDownConf =
          _emaAlpha * rawDownConf + (1 - _emaAlpha) * _smoothedDownConf;
      _smoothedUpConf =
          _emaAlpha * rawUpConf + (1 - _emaAlpha) * _smoothedUpConf;
      ...
      if (_smoothedDownConf > _smoothedUpConf) {
        poseName = '${config.name}_down';
        displayConfidence = downProb;
      } else {
        poseName = '${config.name}_up';
      ...
      // Update notifiers
      currentPose.value =
          _poseEntered ? '${config.name}_down' : '${config.name}_up';
  ...
  void resetCounter() {
    _repetitionCount = 0;
    repetitionCounter.value = 0;
    _poseEntered = false;
    _smoothedDownConf = 0.0;
    _smoothedUpConf = 0.0;
    debugPrint('🔄 Counter reset for ${config.displayName}');
  }
  ...
  void adjustEnterThreshold(double value) {
    ...
    debugPrint(
        '🎚️ Enter threshold for ${config.displayName}: ${enterThreshold.value.toStringAsFixed(1)}');
  }
  ...
  void adjustExitThreshold(double value) {
    ...
    debugPrint(
        '🎚️ Exit threshold for ${config.displayName}: ${exitThreshold.value.toStringAsFixed(1)}');
  }
